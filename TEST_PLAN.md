...
| Context budget | [context-budget.test.ts](extensions/context-budget/context-budget.test.ts): contiguous newest-suffix retention, stable omitted prefixes across mixed-size appends, independent shortened/branch histories, exact UTF-8/part-separator bounds, head/tail retention, metadata and mixed/image pass-through, immutability, fresh evidence/retry after saturated history, cap ordering. | Real SDK context conversion and interactions with compaction, memory, review phases, and multiple extensions. Pure transforms and fake hooks do not prove provider cache hits or net subscription-quota savings. |
...
| Expert discipline and architecture activation | [unit tests](extensions/expert-discipline/expert-discipline.test.ts): independently idempotent, bounded policy/nudge blocks and base-prompt preservation. [SDK tests](scripts/expert-discipline.test.mjs): source-loaded hook, byte-identical provider-visible system prompts across stable turns, skill discovery and explicit invocation from another project, and absent-skill handling. [Package gate](scripts/test_expert_package.mjs): the same boundary from an isolated runtime installation. | Actual model activation, decision quality, comprehension, multi-extension composition, and provider cache behavior. |
...
| Write or review an R function, test, or executable Rmd example | We Use R Damnit: native operations and contract ownership; affected behavior checks, then a mandatory anti-slop consistency pass on changed R source. Inspect complexity and clause-count findings with their contract context. | Scalar validation around every vector operation, a new object system without a semantic need, or reducing scores by hiding unchanged logic. |
| Investigate shared behavior or repeated scalar checks across an R package | Optionally run the bounded consumer-opportunity report, inspect cited functions and admission owners, and choose a class property, simple check, or consumer-owned interface only when needed. | Inferring semantics from identical syntax, creating interfaces or helpers from lint counts, or scanning whole bodies into the agent's context. |
| Review a one-use function defined and directly called inside another R function | Anti-slop reports the local closure for review; keep it only when its lexical scope, state, timing, or callback role has a purpose. | Treating every local function as a defect, or moving the same check across scopes to silence the finding. |
| Change the anti-slop analyzer while its Pi tool is loaded | Run the working-tree R script directly in the mandatory consistency pass; scan newly created untracked files explicitly and distinguish this source result from the loaded tool. | Claiming a cached Pi tool ran the edited analyzer, or suppressing numerical warnings to meet a cosmetic score. |
...
| Bind an already-chosen native library into R | Bindings section of `bioinformatics-rewrites` and R packaging contracts; upstream comparisons where claimed. | Reopening the reuse decision or loading the porting, fusion, and cache sections; inventing SQL/wasm consumers. |
| Package a DuckDB C extension as an R package | R package bindings in `duckdb-c-extension`: provenance, bootstrap, installed DBI load, `R CMD check`. | Reworking extension architecture or vendoring when only the wrapper changes. |
...
| Continue a familiar, already-scoped edit | Use the established context and relevant gate. | Orientation or architecture review as a per-edit/per-commit ritual, or reopening settled choices without relevant changes. |
| Ask about an uncertain design or implementation in another project without naming a skill | The expert-discipline nudge directs proactive reading of architecture-refinement at its discovered location, followed by a bounded learning/review step. | Waiting for an explicit skill command, assuming this repository is the working directory, or treating the nudge as evidence of model adherence. |
| Disable or omit architecture-refinement from the available skill list | Honor the nudge's availability condition; do not claim to have loaded the unavailable skill. | Inventing a skill path or bypassing explicit resource selection. |
| Review a proposed job queue architecture | Architecture refinement: model ownership and delivery promises; challenge a consequential choice with a failure case and evidence; retain domain constraints. | A staged debate, obligatory compromise, or replacing applicable runtime gates with an explanation. |
| Explore an R API whose missing/empty-input semantics are undecided | Use bounded concrete inputs and R-native inspection, retain accepted invariants, and let observations inform a proposed contract; apply We Use R Damnit. | Requiring a complete specification first, importing scalar/object-framework defaults, or silently treating missing and empty values as equivalent. |
| Investigate behavior before a useful causal hypothesis is known | Begin with bounded open observation of representative inputs, outputs, state, or distributions; let a question emerge. | Inventing a design contest or requiring a falsifiable hypothesis before any exploratory work. |
| Apply our R-developer EDA ethos to an unfamiliar non-R subsystem | Inspect concrete inputs and intermediate state with the native tools; compare cases, follow anomalies, and use observations to refine both the question and the next small edit. | Forcing an R rewrite, imposing a statistical workflow on every question, or choosing abstractions before their purpose is understood. |
| A probe reveals that an implementation guess was wrong | Revise the working model and provisional design, then try a small authorized slice or discriminating experiment. | Following an obsolete plan despite contrary evidence, patching only the example, or treating investigation and implementation as one-way phases. |
| Exploration conflicts with an accepted compatibility or data-safety contract | Keep the commitment binding, repair violations within scope, or surface the conflict for an explicit contract decision. | Relabeling a failing requirement as provisional, weakening tests to fit output, or using exploration to authorize destructive/live work. |
| Generate snapshots from a behavior discovered interactively | Distinguish observation from desired behavior; identify an independent requirement, domain rule, validated reference, or owner decision before making a contract test. | Treating generated fixtures or agreement among agents sharing an assumption as independent confirmation. |
| Promote an R experiment that depends on session state | Retain inputs, code, relevant runtime/seed, and reproduce from a clean session or explicit setup before stabilizing package behavior. | Shipping a lucky workspace result, transferring all scratch scaffolding into production, or deleting useful user artifacts. |
| Assess a well-specified component with an established validator | Use the applicable oracle and verification gates, while checking that the objective matches the intended problem. | Dismissing formal specification or useful automation merely because other tasks need exploration. |
| Generated changes and concurrent experiments outrun review capacity | Bound further output, resolve the critical explanatory or verification gap, and keep the next action small. | Increasing agent count or report volume as a substitute for understanding. |
| After an AI-heavy milestone, walk the owner through an upload service they cannot explain | Architecture refinement: reconstruct scoped as-built paths, present a compact model, invite corrections, examine consequential uncertainty, and prioritize supported findings. | A report instead of the requested conversation, a line-level exam, invented historical rationale, treating unfamiliarity or AI provenance as a defect, automatic rewrite, or claims of certified human understanding. |
| Take ownership of a subsystem whose existing decisions remain justified | Explain its model and trade-offs; reason together about an unfamiliar case or plausible modification and investigate discrepancies against evidence. | Treating a document or fluent recitation as transferred understanding, or an ownership change alone as a reason to redesign. |
| Investigate an incident that contradicts the documented recovery behavior | Use the incident as a counterexample to revise the system model and identify a bounded verification or repair action. | Patching only the symptom while leaving the disproven architectural claim intact, or declaring a cause without evidence. |
| Request a batch architecture critique while the owner is unavailable | Proceed with scoped evidence-based analysis and record questions that require owner input. | Forcing an interactive interview, blocking independent analysis, or implying that human understanding has been established. |
| Review an existing system whose design survives the concrete objections | Keep justified behavior and identify any explanation or evidence gaps. | Manufacturing defects or refactoring solely to produce a change. |
| Refine an issue saying “increase cache TTL to fix stale reads” | Separate symptom, causal hypothesis, and requested solution; identify a discriminating experiment and observable acceptance criteria. | Accepting the proposed fix as a requirement, asserting an unverified cause, or requiring a chosen architecture before opening an investigation. |
| Draft repair issues from a retrospective review | Local, bounded drafts tied to evidence, affected contracts, impact, and acceptance criteria; distinguish blockers from follow-ups. | Publishing issues/comments or implementing repairs without the relevant user authorization. |
| Review a demo using customer credentials or shared infrastructure | Scale architecture scrutiny to exposure and consequences; recommend safe checks and containment where needed. | Treating the demo label as a waiver or running a risky live experiment without authorization. |
| Review an isolated, disposable prototype | Lightweight explanation and the highest-impact uncertainty, proportionate to the task. | Production ceremony, exhaustive architecture reconstruction, or an endless interview. |
| Assess a shipping claim with an unresolved recovery invariant | Distinguish investigation/implementation/shipping readiness; report the blocking uncertainty and required verification. | Treating confident prose or happy-path tests as release evidence. |

The architecture-refinement cases cover prospective, exploratory, retrospective,
and issue entry points; developing specifications and fixed commitments; our
R-developer EDA ethos, R-native experimentation, reproducibility, oracle independence,
theory-building, review capacity, event-driven cadence, interaction modes, and
permission boundaries. Review them alongside the mechanical README/familiar-edit
cases to check non-matches. SDK and package checks establish provider-visible
activation guidance, discovery, explicit invocation, and shipped references, not
actual model routing, design correctness, or retained human understanding.
