...
- Upgrade RLM to the ordered Luna < Terra < Sol capability/cost policy. Root and child defaults are Luna; static `rlm` guidelines select tiers by semantic difficulty, `thinking`/`subThinking` resolve from deterministic tier/role/context-kind/size policy unless explicit, `--thinking` reaches child Pi calls, and depth>0 planner/worker/synthesis calls use the configured child model/effort. Complete byte-stable role instructions keep runtime task, observations, and counters in user prompts without claiming provider-independent cache hits.
...
- Add a repository-vendored, Tree-sitter-only R/C anti-slop analyzer with a Pi `anti_slop` tool and `/anti-slop` command. It scans source files or whole directories (Git-tracked sources when available), flags structured redundant final returns and terminating `else` branches, identical pure-condition branches, no-op R rethrow handlers, duplicate terminating guards, private R-helper direct-call counts, single-use predicate chains, translated scalar-validator layers, path-security checks without a real privilege boundary, cyclomatic complexity of 15 or greater, boolean-expression sprawl even when moved behind a local alias, ambiguous `length()` truthiness, empty alternatives, and C runtime assertions that can abort an embedded host; JSON configuration controls rule severity without parser fallback. An explicitly requested installed Jarl run contributes validated, namespaced R diagnostics without becoming a bundled dependency or parser fallback; every result exposes engine state, disabled rules, total findings, and truncation so reviewers can distinguish partial or suppressed scans from clean full-scope evidence.
