...
- `r-one-use-local-closure`: a function bound directly in a braced function body and called directly once in that body, with no other syntactic references to its name. Callbacks, escaping closures, rebinding, and repeated calls are excluded; a reported closure can still have a real lexical-state purpose.
...
- `r-conditional-sprawl`: any maximal boolean expression with more than three atomic `&&`, `||`, `&`, or `|` clauses, including equivalent direct clauses supplied through `all(...)`, `any(...)`, `all(c(...))`, or `any(c(...))`, whether used as a condition, assigned to a local alias, passed as an argument, or returned. Review its decision structure on the mandatory consistency pass; renaming or mechanically re-encoding clauses is not a simplification.
...
- `r-cyclomatic-complexity`: a function whose cyclomatic complexity is 15 or greater, triggering mandatory second-pass inspection rather than a required score reduction. Counting uses `cyclocomp`-compatible contributions for `if`, `for`, `while`, `repeat`, `&&`, and `||`; nested function bodies are scored independently, while `&`, `|`, and `ifelse()` do not add paths.
